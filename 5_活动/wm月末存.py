...
class DatabaseQuery:
    def __init__(self, host: str, port: int, user: str, password: str,
                 ...
                 site_id: int = 1000, start_date: str = '2025-04-23', end_date: str = '2025-04-30',
    ...
    def mongo_betting_stats(self, use_date_column: bool = False) -> pd.DataFrame:
        ...
        if df.empty:
            return pd.DataFrame(
                columns=['日期', '会員ID', '投注次数', '有效投注', '会员输赢'] if use_date_column else ['会员ID',
                                                                                                        '投注次数',
                                                                                                        '有效投注',
                                                                                                        '会员输赢'])
...
def work(db_query: DatabaseQuery) -> pd.DataFrame:
    ...
    deposit_rules = [
        (5000, 28, 0.5, 1.0, 2.0),
        (10000, 88, 0.5, 1.0, 2.0),
        (30000, 208, 0.5, 1.0, 2.0),
        (50000, 388, 0.5, 1.0, 2.0),
        (100000, 888, 0.5, 1.0, 2.0),
        (300000, 2088, 0.5, 1.0, 2.0),
        (500000, 3888, 0.5, 1.0, 2.0)
    ]

    # 根据存款金额计算每日彩金
    def calculate_daily_bonus(deposit):
        for threshold, bonus, _, _, _ in deposit_rules:
            if deposit >= threshold:
                return bonus
        return 0

    # 应用每日彩金
    result_df['每日彩金'] = result_df['存款'].apply(calculate_daily_bonus)

    # 计算每个会员的参与天数（有存款的日期数）
    participation_days = result_df[result_df['存款'] > 0][['会员ID', '日期']].groupby('会员ID').nunique()[
        '日期'].reset_index()
    participation_days.columns = ['会员ID', '参与天数']

    # 获取每个会员的最高每日彩金
    max_bonus = result_df[result_df['每日彩金'] > 0][['会员ID', '每日彩金']].groupby('会员ID').max().reset_index()

    # 合并参与天数和最高彩金
    bonus_df = max_bonus.merge(participation_days, on='会员ID', how='left').fillna({'参与天数': 0})

    # 计算倍数金额
    def calculate_multiplier_bonus(row):
        max_bonus = row['每日彩金']
        days = row['参与天数']
        three_day_bonus = max_bonus * 0.5 if days >= 3 else 0
        five_day_bonus = max_bonus * 1.0 if days >= 5 else 0
        eight_day_bonus = max_bonus * 2.0 if days >= 8 else 0
        return pd.Series({
            '3日参与倍数金额': three_day_bonus,
            '5日参与倍数金额': five_day_bonus,
            '8日参与倍数金额': eight_day_bonus
        })

    # 应用倍数金额计算
    bonus_df[['3日参与倍数金额', '5日参与倍数金额', '8日参与倍数金额']] = bonus_df.apply(calculate_multiplier_bonus,
                                                                                         axis=1)

    # 合并到 result_df
    result_df = result_df.merge(bonus_df[['会员ID', '3日参与倍数金额', '5日参与倍数金额', '8日参与倍数金额']],
                                on='会员ID', how='left').fillna(0)
