...
def generate_0_Ultimate_Analysis_html():
   ...
   for sheet in sheets:
       ...
       if x_axis_col == "日期":
           # 尝试解析日期，处理文本格式
           sheet_df[x_axis_col] = pd.to_datetime(sheet_df[x_axis_col], errors='coerce', format='%Y-%m-%d')
           # 提取年月和日
           sheet_df['year_month'] = sheet_df[x_axis_col].dt.strftime('%Y-%m')
           sheet_df['day'] = sheet_df[x_axis_col].dt.strftime('%d')
           # 获取唯一的年月和日组合
           sheet_date_labels = sheet_df[[x_axis_col, 'year_month', 'day']].dropna().drop_duplicates()
           # 用于 Chart.js 的标签：[{day: 'DD', year_month: 'YYYY-MM'}]
           sheet_date_labels = [
               {'day': row['day'], 'year_month': row['year_month']}
               for _, row in sheet_date_labels.iterrows()
           ]
       else:
       ...
       if sheet == "人数":
           ...
           for i, column in enumerate(y_left_cols):
               if column in value_columns:
                   data = sheet_df.groupby(x_axis_col)[column].sum().reindex(sheet_df[x_axis_col].dropna().unique()).fillna(0).tolist()
...
function createChart(canvasId, data, type) {
   ...
   var isDateAxis = canvasId !== '存款-chart' && canvasId !== '取款-chart'; // 判断是否为日期轴
   var options = {
       responsive: true,
       ...
       scales: {
           x: {
               type: 'category',
               labels: isDateAxis ? data.labels.map(item => item.day) : data.labels, // 上层显示“DD”
               ticks: { color: '#fff' },
               // 添加第二层轴（年月）
               afterBuildTicks: function(scale) {
                   if (isDateAxis) {
                       ...
                       data.labels.forEach((label, index) => {
                           if (!yearMonthTicks[label.year_month]) {
                               yearMonthTicks[label.year_month] = [];
                           }
                           yearMonthTicks[label.year_month].push(index);
                       });
                       scale.ticks = scale.ticks.map((tick, index) => {
                           for (var ym in yearMonthTicks) {
                               if (yearMonthTicks[ym].includes(index)) {
                                   if (yearMonthTicks[ym].indexOf(index) === Math.floor(yearMonthTicks[ym].length / 2)) {
                                       // 在年月的中间位置显示
                                       yearMonths[index] = ym;
                                   } else {
                                       yearMonths[index] = '';
                       ...
                       scale._yearMonthTicks = yearMonths; // 存储年月标签
                   }
               },
               afterTickToLabelConversion: function(scale) {
                   if (isDateAxis) {
                       ...
                       scale.ticks.forEach((tick, index) => {
                           if (scale._yearMonthTicks[index]) {
                               tick.label = [tick.label, scale._yearMonthTicks[index]]; // 两层标签
                           }
                       });
                   }
               },
...
function updateTable(data, filteredData, xAxisCol0170Col, filterCol) {
   ...
   var tableData = allData[currentSheet].labels.map(label => {
       var xAxis = xAxisCol === '日期' ? (label.year_month + '-' + label.day) : label; // 合并为 YYYY-MM-DD
...
function filterData(filterKey, element) {
   ...
   sheetData.labels.forEach(label => {
       var key = xAxisCol === '日期' ? (label.day + '|' + label.year_month) : label; // 使用 day|year_month 作为键
       groupedData[key] = {};
       sheetData.datasets.forEach(dataset => {
           groupedData[key][dataset.label] = 0; // Initialize with 0
       });
   });
   ...
   filteredData.forEach(row => {
       var xAxis = row[xAxisCol];
       // 解析日期，处理文本格式
       var yearMonth = xAxis ? xAxis.substring(0, 7) : ''; // YYYY-MM
       var day = xAxis ? xAxis.substring(8, 10) : ''; // DD
       var key = xAxisCol === '日期' ? (day + '|' + yearMonth) : xAxis;
       if (groupedData[key]) {
           sheetData.datasets.forEach(dataset => {
               ...
               groupedData[key][dataset.label] = (groupedData[key][dataset.label] || 0) + value;
           });
       }
   });
   ...
   sheetData.datasets.forEach(dataset => {
       ...
       sheetData.labels.forEach(label => {
           ...
           let key = xAxisCol === '日期' ? (label.day + '|' + label.year_month) : label;
           let value = groupedData[key] ? groupedData[key][dataset.label] || 0 : 0;
           datasetDataPoints.push({ x: label.day || label, y: value });
       });
   ...
   var newData = {
       labels: sheetData.labels, // Keep original labels for context
