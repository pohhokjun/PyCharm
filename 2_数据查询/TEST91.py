...
def process_collection(col_name, mongo_uri, db_name, start_time, end_time, site_id):
    """处理单个MongoDB集合的聚合查询"""
    client = pymongo.MongoClient(mongo_uri, maxPoolSize=50)
    collection = client[db_name][col_name]
    pipeline = [
        {"$match": {"flag": 1, "settle_time": {"$gte": start_time, "$lte": end_time}, "site_id": site_id}},
        {
            "$group": {
                "_id": {
    ...
    try:
        df = pd.DataFrame([
            ...
            } for doc in collection.aggregate(pipeline, cursor={}, batchSize=1000)
        ])
    finally:
        client.close()
    return df
...
class DatabaseQuery:
    def __init__(self, host: str, port: int, user: str, password: str,
        ...
        self.mongo_client = pymongo.MongoClient(self.mongo_uri, maxPoolSize=50)
    ...
    def query_mongo_betting_stats(self) -> pd.DataFrame:
        """查询 MongoDB 投注统计数据
        建议为 pull_order 集合创建索引：
        db.pull_order.createIndex({"flag": 1, "settle_time": 1, "site_id": 1})
        db.pull_order.createIndex({"member_id": 1, "game_type": 1})
        """
        ...
        with Pool(processes=4) as pool:
            ...
            results = list(tqdm(pool.imap(partial_process, collections), total=len(collections), desc="Processing collections"))

        # 合并结果
        type_data = pd.concat(results, ignore_index=True)
        if type_data.empty:
            return pd.DataFrame(columns=['date', 'member_id', '投注次数', '有效投注', '会员输赢'])
        ...
        type_data_valid_pivot.columns = type_data_valid_pivot.columns[:2].tolist() + [
            game_type_mapping.get(col, col) for col in type_data_valid_pivot.columns[2:]
        ]

        # 合并数据并过滤
        daily_data = pd.merge(member_daily_bet, type_data_valid_pivot, on=['date', 'member_id'], how='outer')
