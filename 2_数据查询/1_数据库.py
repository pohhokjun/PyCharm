...
def get_table_time_range_and_count_to_excel(host, port, user, password, filename='数据库.xlsx'):
   ...
   Column Names 分列显示，删除 Column Types，每行数据后插入空行。
   ...
   try:
       ...
       # 插入空行
       empty_row = pd.DataFrame([[None] * len(columns)], columns=columns)
       df_with_empty = pd.concat([df, empty_row]).sort_index(kind='merge').reset_index(drop=True)
       df_with_empty = df_with_empty.iloc[:-1]  # 移除最后多余的空行

       # 导出到 Excel 文件
       with pd.ExcelWriter(filename, engine='openpyxl') as writer:
           df_with_empty.to_excel(writer, index=False, sheet_name='Sheet1')
