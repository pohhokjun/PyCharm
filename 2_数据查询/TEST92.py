...
class DatabaseQuery:
   def __init__(self, host: str, port: int, user: str, password: str,
                ...
                site_id: int = 1000, start_date: str = '2025-04-20', end_date: str = '2025-04-25',
   ...
   def _10_promotion(self) -> pd.DataFrame:
       ...
       WHERE
           ...
           AND a1_ad_2.group_name IN ('推广7部')
   ...
   def _3_mongo_last_bet_time(self) -> pd.DataFrame:
       ...
       pipeline = [
           {"$match": {
               ...
               "bet_time": {"$gte": self.start_time, "$lte": self.end_time},
       ...
       if not df.empty:
           df = df.sort_values(by=['会员ID', '最后下注时间'], ascending=False)
           df = df.drop_duplicates(subset=['会员ID'], keep='first')
...
def work(db_query: DatabaseQuery) -> pd.DataFrame:
   ...
   result_df = (db_query._10_promotion()
                ...
                .merge(db_query._3_first_deposit(), on='会员ID', how='inner')
                .merge(db_query._3_mongo_last_bet_time(), on='会员ID', how='inner')
                .merge(db_query._2_member_stats_history(), on='会员ID', how='inner')
                )
   return result_df
