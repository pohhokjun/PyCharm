...
def export_data():
    ...
    # 创建 Excel 文件
    output_file = f"{FOLDER_PATH}/账户余额.xlsx"
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        df2.to_excel(writer, sheet_name='汇总数据', index=False)  # 汇总数据写在第一页
        df1.to_excel(writer, sheet_name='详细数据', index=False)  # 详细数据写在第二页
...
async def main():
    """主程序：定时执行数据导出、发送和删除"""
    while True:
        await asyncio.sleep(get_time_until(20, 41))
