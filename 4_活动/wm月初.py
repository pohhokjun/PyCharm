...
class DatabaseQuery:
    def __init__(self, host: str, port: int, user: str, password: str,
                 ...
                 site_id: int = 1000, start_date: str = '2025-05-01', end_date: str = '2025-05-08',
...
# 奖励表定义（基于活动规则）
BONUS_TABLE = [
    {'sports_bet': 3888, 'other_bet': 5888, 'base_bonus': 12, '5_day_sign': 18, '8_day_sign': 28},
    {'sports_bet': 12888, 'other_bet': 18888, 'base_bonus': 38, '5_day_sign': 58, '8_day_sign': 88},
    {'sports_bet': 38888, 'other_bet': 58888, 'base_bonus': 118, '5_day_sign': 188, '8_day_sign': 238},
    {'sports_bet': 88888, 'other_bet': 128888, 'base_bonus': 258, '5_day_sign': 388, '8_day_sign': 628},
    {'sports_bet': 188888, 'other_bet': 288888, 'base_bonus': 588, '5_day_sign': 688, '8_day_sign': 988},
    {'sports_bet': 388888, 'other_bet': 588888, 'base_bonus': 1188, '5_day_sign': 1288, '8_day_sign': 2288},
    {'sports_bet': 688888, 'other_bet': 1088888, 'base_bonus': 2088, '5_day_sign': 2588, '8_day_sign': 3988},
    {'sports_bet': 1188888, 'other_bet': 1888888, 'base_bonus': 3588, '5_day_sign': 3888, '8_day_sign': 7088},
    {'sports_bet': 1888888, 'other_bet': 2888888, 'base_bonus': 5888, '5_day_sign': 6888, '8_day_sign': 9888},
]

# 特权加赠表
PRIVILEGE_BONUS_TABLE = [
    {'deposit': 2000, 'bonus_rate': 0.10},
    {'deposit': 10000, 'bonus_rate': 0.15},
    {'deposit': 30000, 'bonus_rate': 0.20},
    {'deposit': 50000, 'bonus_rate': 0.25},
    {'deposit': 100000, 'bonus_rate': 0.30},
]

def calculate_bonus(row, bonus_table, privilege_table):
    """计算基础奖励、投注特权加赠、5日签到和8日满签奖励"""
    sports_bet = row.get('体育有效投注', 0) + row.get('电竞有效投注', 0)
    other_bet = row.get('真人有效投注', 0) + row.get('彩票有效投注', 0) + \
                row.get('棋牌有效投注', 0) + row.get('电子有效投注', 0) + row.get('捕鱼有效投注', 0)
    deposit = row.get('存款', 0)
    ...
    for tier in bonus_table:
        if sports_bet >= tier['sports_bet'] or other_bet >= tier['other_bet']:
            base_bonus = max(base_bonus, tier['base_bonus'])
            bonus_5_day = max(bonus_5_day, tier['5_day_sign'])
            bonus_8_day = max(bonus_8_day, tier['8_day_sign'])

    # 计算特权加赠
    for tier in privilege_table:
        if deposit >= tier['deposit']:
            privilege_bonus = base_bonus * tier['bonus_rate']

    return pd.Series({
        '基础奖励': base_bonus,
        '投注特权加赠': privilege_bonus,
        '5日签到': bonus_5_day if row.get('签到天数', 0) >= 5 else 0,
        '8日满签': bonus_8_day if row.get('签到天数', 0) >= 8 else 0
    })


def get_signin_data(db_query):
    """计算签到天数：当天有投注即算签到"""
    betting_stats = db_query.mongo_betting_stats(use_date_column=True)
    betting_stats = betting_stats[betting_stats['有效投注'] > 0]
    signin_df = betting_stats.groupby('会员ID')['日期'].nunique().reset_index()
    signin_df.columns = ['会员ID', '签到天数']
    return signin_df
...
def work(db_query):
    """执行查询并合并结果，添加奖励计算"""
    result_df = (db_query._6_member_stats_period(use_date_column=True)
                 .merge(db_query.mongo_betting_stats(use_date_column=True), on=['会员ID', '日期'], how='inner'))

    signin_df = get_signin_data(db_query)
    result_df = result_df.merge(signin_df, on='会员ID', how='left')
    result_df['签到天数'] = result_df['签到天数'].fillna(0).astype(int)

    bonus_df = result_df.apply(lambda row: calculate_bonus(row, BONUS_TABLE, PRIVILEGE_BONUS_TABLE), axis=1)
    result_df = pd.concat([result_df, bonus_df], axis=1)

    final_columns = list(result_df.columns)
    for col in ['基础奖励', '投注特权加赠', '5日签到', '8日满签']:
