...
def export_data():
   ...
   SELECT 
       ...
       ANY_VALUE(w.available_money) AS 账户余额,
       ANY_VALUE(w.usdt_money) AS USDT余额,
       ANY_VALUE(w.valet_money) AS 代客金额,
       ANY_VALUE(w.agent_money) AS 代理金额
   FROM 
   ...
   # 创建 Excel 文件
   output_file = f"{FOLDER_PATH}/账户余额_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
def send_file(bot, file_path, chat_id, summary_text):
   """通过 Telegram 发送文件和文字作为 caption"""
   try:
       with open(file_path, 'rb') as file:
           asyncio.run(bot.send_document(chat_id=chat_id, document=file, caption=summary_text))
...
async def main():
   """主程序：定时执行数据导出、发送和删除"""
   while True:
       await asyncio.sleep(get_time_until(20, 50))
       try:
           output_file, summary_text = export_data()
           await asyncio.get_event_loop().run_in_executor(None, lambda: send_file(bot, output_file, CHAT_ID, summary_text))
